...
export default function CarPage({ params }) {
  ...
  if (loading) {
    return <div className="max-w-4xl mx-auto px-4 py-12 text-center"><p className="text-gray-500">{lang === 'ru' ? 'Загрузка...' : 'Loading...'}</p></div>;
  }
  ...
  const statusLabel = car.car_status === 'Owned' ? (lang === 'ru' ? 'Сейчас владею' : 'Currently own') : (lang === 'ru' ? 'Владел ранее' : 'Previously owned');
  const visibilityLabel = car.visibility === 'Public' ? (lang === 'ru' ? 'Видно всем' : 'Visible to everyone') : car.visibility === 'Visible for registered users' ? (lang === 'ru' ? 'Видно зарегистрированным' : 'Visible to registered users') : (lang === 'ru' ? 'Видно только мне' : 'Only visible to me');

  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <a href={`/${lang}/garage`} className="text-blue-700 no-underline">← {lang === 'ru' ? 'Гараж' : 'Garage'}</a>
      <h1 className="text-3xl font-bold mt-4">{car.name || (lang === 'ru' ? 'Без названия' : 'Untitled')}</h1>
      <div className="flex flex-wrap gap-3 mt-3 text-sm text-gray-500">
        <span className={car.car_status === 'Owned' ? 'type-badge' : 'type-badge !bg-gray-200 !text-gray-600'}>{statusLabel}</span>
        <span>{visibilityLabel}</span>
      </div>
      <div className="mt-4">
        <a href={`/${lang}/garage/${id}/edit`} className="text-blue-700 text-sm no-underline hover:underline">✏️ {lang === 'ru' ? 'Редактировать' : 'Edit'}</a>
      </div>
      ...
      {car.notes && (
        <div className="mt-10">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 text-sm text-gray-700 leading-relaxed whitespace-pre-wrap italic">{car.notes}</div>
        </div>
      )}
