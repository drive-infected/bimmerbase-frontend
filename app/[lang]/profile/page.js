...
export default function ProfilePage({ params }) {
  ...
  useEffect(() => {
    ...
    setUser(userData);
    fetchCarsCount(jwt, userData.id);
    setLoading(false);
  }, []);

  const fetchCarsCount = async (jwt, userId) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/user-cars?filters[users_permissions_user][id][$eq]=${userId}`,
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="flex gap-4">
        ...
        <button
          ...
          className="px-6 py-3 rounded-lg border border-red-300 text-red-600 hover:bg-red-50 transition-colors"
        >
